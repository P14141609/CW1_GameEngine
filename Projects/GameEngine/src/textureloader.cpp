...
int TextureLoader::loadBMP(const std::string ksTextureFile, GLuint &texture)  //const char* location
{
	...
	std::ifstream file(ksTextureFile, std::ios::binary);
	if (!file)
	{
		std::cerr << "Bitmap \"" << ksTextureFile << "\" not found." << std::endl;
		return 1;
	}
	else
	{
		std::cerr << "Bitmap \"" << ksTextureFile << "\" successfully loaded." << std::endl;
	}
	...
	{
		std::cerr << "File \"" << ksTextureFile << "\" Not a bitmap file (wrong header)" << std::endl;
		return 2;
	}

	if (bmpInfo->biBitCount != 24)
	{
		std::cerr << "File \"" << ksTextureFile << "\" bits per pixel must be 24" << std::endl;
		return 2;
	}
	...
	{
		std::cerr << "File \"" << ksTextureFile << "\" more than one plane" << std::endl;
		return 2;
	}
	...
	std::cerr << "Texture \"" << ksTextureFile << "\" successfully loaded." << std::endl;
